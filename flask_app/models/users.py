...
class User:
    ...
    def save(cls, formulario):
        #Formulario= {firts_name = 'Elena', last_name = 'De Troya', email = 'Elenadet@coding.com', password = 'Elena123456', }
        query = "INSERT INTO users (first_name, last_name, email, password) VALUES(%(first_name)s, %(last_name)s, %(email)s, %(password)s )"
        resut = connectToMySQL('esquema_muro_privado').query_db(query, formulario)
    ...
    def get_by_email(cls, formulario):
        ...
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('esquema_muro_privado').query_db(query, formulario) #SELECT me regresa una lista
    ...
    def get_all(cls):
        ...
        results = connectToMySQL('esquema_muro_privado').query_db(query) #Regresa una lista de diccionarios
        #results = [
